...
def AI_Product_Analysis(text):
    ans = groq_inference("Analyze the products mentioned in the following news article in 400 words or fewer. Focus on their features, market relevance, and potential impact for a company's market planning: " + text)
    return ans
...
def AI_Company_Summary(news_summaries):
    ans = groq_inference("Analyze the following combined summaries about multiple products. Provide a detailed summary of each product individually, clearly outlining their features, market relevance, and competitive advantages, so this information can be used to analyze competitor products: " + news_summaries)
    return ans
...
def AI_Analysis(Product_analysis,Compitetiors_analysis):
    ...
    response = groq_inference(system_prompt)
...
def summary_name(otp,product):
  ...
  summ_data = [analysis_name(i,product) for i in text_data]
